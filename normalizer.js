...
var Normalizer = (function () {
    ...
    Normalizer.prototype.denormalize = function () {
        var binaryInput = [];
        var binaryOutput = [];
        for (var i in this.dataset) {
            var row = this.dataset[i];
            ...
            for (var prop in row) {
                ...
                switch (meta.type) {
                    case 'number':
                        bitsArr = [this.goBack(meta.min, meta.max, value)];
                        break;
    ...
    Normalizer.prototype.goBack = function (min, max, value) {
        var num = (value *(max - min))+min;
        return Number((num).toFixed(6));
    };
